...
// 지출 내역 업데이트
export const updateSpendReport = createAsyncThunk<Report, Report>(
  "spend/updateSpendReport",
  async (report: Report) => {
...
const spendSlice = createSlice({
  name: "spend", //slice 이름 정의, Redux 상태의 키
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      addSpendReport.fulfilled,
      // 새 지출 내역 추가 비동기 작업 요청 완료 왜 Report[] ?
      (state, action: PayloadAction<Report>) => {
        console.log("Adding spend report:", action.payload);
        // state.push를 통해 배열에 새 내역 추가
        state.push(action.payload);
        console.log("State after adding report:", state);
      }
    );
    builder.addCase(deleteSpendReport.fulfilled, (state, action) => {
      return state.filter((report) => report.id !== action.payload);
    });

    builder.addCase(updateSpendReport.fulfilled, (state, action) => {
