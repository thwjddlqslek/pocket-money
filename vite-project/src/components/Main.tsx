...
const Main: React.FC = () => {
  ...
  const years : number[] = Array.from({ length: 3 }, (_, index) => currentYear - index);
    ...
    <Header totalIncome={totalIncome} totalSpend={totalSpend}/>
      <m.MainContainer>
        <m.PocketContainer>
          ...
          <m.TotalContainer>
            <div>수입 : <m.ColorText color="#01A361">{totalIncome.toLocaleString()} 원</m.ColorText></div>
            <div>지출 : <m.ColorText color="#E12939">{totalSpend.toLocaleString()} 원</m.ColorText></div>
          </m.TotalContainer>
          <m.ReportContainer>
            <div>
              {incomeReports.map((report, index) => (
                <IncomeReport key={index} date={report.date} text={report.text} number={report.number} />
              ))}
            </div>
            <div>
              {spendReports.map((report, index) => (
                <SpendReport key={index} date={report.date} text={report.text} number={report.number} />
              ))}
            </div>
          </m.ReportContainer>
        </m.PocketContainer>
